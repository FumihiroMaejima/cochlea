...
class BannerBlocksResource extends JsonResource
{
    ...
    public static function toArrayForGetCollectionList(Collection $collection)
    {
        ...
        foreach ($collection as $item) {
            $item->{BannerBlocks::START_AT} = TimeLibrary::format($item->{BannerBlocks::START_AT});
            $item->{BannerBlocks::END_AT} = TimeLibrary::format($item->{BannerBlocks::END_AT});
    ...
    public static function toArrayForGetTextAndValueList(Collection $collection)
    {
        ...
        foreach ($collection as $item) {
            ...
            $role = [
                self::RESOURCE_KEY_TEXT => $item->{BannerBlocks::NAME},
                self::RESOURCE_KEY_VALUE => $item->{BannerBlocks::ID},
            ];
            // 多次元配列の中の連想配列を格納
    ...
    public static function toArrayForBulkInsert(array $resouce)
    {
        ...
        foreach ($resouce as $key => $item) {
            ...
            if ($key !== 0) {
                $response[] = [
                    BannerBlocks::NAME       => $item[0],
                    BannerBlocks::ORDER      => $item[1],
                    BannerBlocks::START_AT   => $item[2],
                    BannerBlocks::END_AT     => $item[3],
                    BannerBlocks::CREATED_AT => $dateTime,
                    BannerBlocks::UPDATED_AT => $dateTime,
                ];
            }
        }
